...
**Память.** Neo4j потребляет много оперативной памяти: для типичного инстанса с порядка **2,5 млн постов** в графе обычно требуется около **8 ГБ RAM** на один экземпляр Neo4j (плюс запас на пиковые операции GDS). Закладывайте ресурсы хоста или лимиты контейнера с учётом этой оценки.
