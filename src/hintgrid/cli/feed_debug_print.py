...
def print_feed_inclusion_explanation(explanation: FeedInclusionExplanation) -> None:
    ...
    if sc:
        ...
        st.add_row("share_i", f"{sc['share_i']:.6f}")
        st.add_row("norm_pc_size", f"{sc['norm_pc_size']:.6f}")
        st.add_row("local_raw", f"{sc['local_raw']:.6f}")
        st.add_row("global_raw", f"{sc['global_raw']:.6f}")
        st.add_row("popularity_contrib", f"{sc['popularity_contrib']:.6f}")
        ...
        st.add_row("weighted_pc_share", f"{sc['weighted_pc_share']:.6f}")
        st.add_row("weighted_pc_size", f"{sc['weighted_pc_size']:.6f}")
