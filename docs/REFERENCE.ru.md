...
Пошаговая установка на сервер Mastodon (пользователь ОС `hintgrid`, **создание в PostgreSQL роли только на чтение** для базы Mastodon, пути, команды `systemctl`, интеграция Redis с Mastodon) описана в [README.ru.md](../README.ru.md) в разделе **«Установка на сервере Mastodon (systemd)»**.
