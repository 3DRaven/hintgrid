...
| leiden_diagnostics_enabled | — | `HINTGRID_LEIDEN_DIAGNOSTICS` | `false` | `true` | Расширенные метрики графа и результата Leiden в логе (без второго прогона алгоритма) |
...
| apoc_batch_size | `--apoc-batch-size` | `HINTGRID_APOC_BATCH_SIZE` | `10000` | `10000` | Размер батча для `apoc.periodic.iterate`: INTERESTED_IN, узлы UserCommunity/PostCommunity и BELONGS_TO, cleanup, **батчевое удаление** исходящих `SIMILAR_TO` при полной пересборке похожести, aggressive pruning и др. (1–100000). Построение `SIMILAR_TO` использует `similarity_iterate_batch_size` |
