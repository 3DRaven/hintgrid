...
Пошаговая установка на сервер Mastodon (пользователь ОС `hintgrid`, **создание в PostgreSQL роли только на чтение** для базы Mastodon, **Neo4j в Docker** через `deploy/docker-compose.neo4j.yml`, пути, команды `systemctl`, интеграция Redis с Mastodon) описана в [INSTALL.ru.md](../INSTALL.ru.md) и [INSTALL.md](../INSTALL.md) (English).
