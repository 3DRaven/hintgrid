...
После полного сохранения чекпоинта (`fasttext_v{N}.bin`) при включённом квантовании дополнительно записывается сжатый файл `fasttext_v{N}.q.bin`: векторы словаря и n-грамм проходят product quantization (библиотека **compress-fasttext**; размерность шага задаётся `quantize_qdim`). В режиме инференса при наличии `.q.bin` загружается он (формат KeyedVectors), иначе — полная модель.
...
| progress_output | — | `HINTGRID_PROGRESS_OUTPUT` | `auto` | `auto` | Режим индикаторов: `auto` (TTY — Rich, иначе построчный вывод в лог для journald), `rich`, `plain` |
...
| apoc_batch_size | `--apoc-batch-size` | `HINTGRID_APOC_BATCH_SIZE` | `10000` | `10000` | Размер батча для `apoc.periodic.iterate`: INTERESTED_IN, узлы UserCommunity/PostCommunity и BELONGS_TO, cleanup и др. (1–100000). Построение `SIMILAR_TO` использует `similarity_iterate_batch_size` |
