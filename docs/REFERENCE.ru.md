...
- Один запрос формирует персональную ленту по интересам сообществ, сигналу популярности, свежести и (опционально) PageRank по графу `SIMILAR_TO`.
- Веса контролируются параметрами `personalized_*` и параметрами популярности (ниже).
- Если персональных результатов нет, применяется cold start (посты с эмбеддингом по тому же скору без интереса).

**Семантика «local» в этом разделе** — данные графа Neo4j на инстансе (взвешенные счётчики входящих рёбер к посту), **не** флаг Mastodon `isLocal` и не фильтр `local_only` публичной ленты.

#### Популярность: режимы и параметры (Cypher)

| Параметр | Смысл |
|----------|--------|
| `$likes_weight`, `$reblogs_weight`, `$replies_weight`, `$bookmark_weight` | Веса для типов рёбер `FAVORITED`, `REBLOGGED`, `REPLIED`, `BOOKMARKED` при расчёте локального сырья (если тип включён в политику `rel_types`; иначе соответствующее слагаемое не считается). |
| `$gf`, `$gr`, `$grp` | Веса полей узла `Post`: `totalFavourites`, `totalReblogs`, `totalReplies` (глобальный композит; сумма трёх весов = 1.0). |
| `$popularity_smoothing` | Сглаживание внутри `log10` для локального сигнала (`s_loc`), должно быть > 0. |
| `$global_popularity_smoothing` | Сглаживание внутри `log10` для глобального композита (`s_glob`), > 0. |
| `$blend_local`, `$blend_global` | Только для режима `blended`: веса на `log10(local_raw + s_loc)` и `log10(global_raw + s_glob)` (сумма = 1.0). |

**Локальное сырьё** (пример, когда учитываются все четыре типа):
...
**Глобальное сырьё** (поля узла `Post`, подставляются из статистики статусов):
...
**Вклад популярности `popularity_contrib`** (один из вариантов по `HINTGRID_FEED_POPULARITY_MODE`):

```cypher
// local:   log10(local_raw + $popularity_smoothing)
// global:  log10(global_raw + $global_popularity_smoothing)
// blended: toFloat($blend_local) * log10(local_raw + $popularity_smoothing)
//          + toFloat($blend_global) * log10(global_raw + $global_popularity_smoothing)
```

Итоговый член скора ленты: `popularity_contrib * $popularity_weight` (отдельно от PageRank и языкового CASE).

**Статистика по сообществам** (`show_post_community_stats`) использует те же `local_raw`, `global_raw` и `popularity_contrib` для «Popular» и среднего по постам — та же модель, что и лента.

#### Запрос ленты для пользователя (с холодным стартом)

**Персонализированная лента** (упрощённо; фактическая сборка в `feed_personalized_queries` / пайплайне):

Перед матчем постов по рёбрам интереса запрашиваются агрегаты по тому же `uc`: `sum_i = sum(i.score)` по всем `(uc)-[:INTERESTED_IN]->(pc)` и `max_pc_size = max(toFloat(coalesce(pc.size, 0)))` среди этих PC. Для строки кандидата `(p, pc, i)` задаются `share_i = i.score / sum_i` (при `sum_i > 0`) и `norm_pc_size = pc.size / max_pc_size` (при `max_pc_size > 0`). Член `i.score * interest_weight` **не** входит в итоговый score: вместо него используются `share_i` и `norm_pc_size` с весами `feed_pc_share_weight` и `feed_pc_size_weight`. При пересборке интересов `i.score` в UC — это `weight / max_weight` по сообществу (шкала [0,1] относительно сильнейшего PC), **не** вероятность и **не** сумма 1 по всем PC; после деления на `sum_i` получается доля «массы интереса» данного PC среди всех интересов пользователя. Мягкая диверсификация **не** задаёт жёстких квот по PC. Сумма `feed_pc_share_weight + feed_pc_size_weight + personalized_popularity_weight + personalized_recency_weight` должна быть 1.0 (плюс отдельно языковой CASE и при необходимости PageRank). Снизить влияние графового ранга постов можно через `pagerank_weight` / `pagerank_enabled`.

**Публичные ленты** (`generate_public_feed`): на агрегат `community_interest = sum(i.score)` по матчу накладывается скаляр `feed_pc_share_weight + feed_pc_size_weight` (тот же суммарный «бюджет интереса», что и два PC-члена вместе в персональной ветке).

```cypher
// Поток: агрегаты по uc → матч постов → local_raw / global_raw / popularity_contrib (как в feed_queries)
// → share_i, norm_pc_size → итоговый score (фрагменты ниже — ключевые отличия от сырого i.score)

MATCH (u:User {id: $user_id})-[:BELONGS_TO]->(uc:UserCommunity)
MATCH (uc)-[i_agg:INTERESTED_IN]->(pc_agg:PostCommunity)
WITH u, uc, sum(i_agg.score) AS sum_i, max(toFloat(coalesce(pc_agg.size, 0))) AS max_pc_size
MATCH (uc)-[i:INTERESTED_IN]->(pc:PostCommunity)<-[:BELONGS_TO]-(p:Post)
WHERE p.createdAt > datetime() - duration({days: $feed_days})

// ... далее цепочка WITH с local_raw, global_raw, popularity_contrib, затем:
// share_i = CASE WHEN sum_i > 0.0 THEN toFloat(i.score) / toFloat(sum_i) ELSE 0.0 END
// norm_pc_size = CASE WHEN max_pc_size > 0.0 THEN toFloat(coalesce(pc.size,0)) / max_pc_size ELSE 0.0 END
// score = share_i * $feed_pc_share_weight + norm_pc_size * $feed_pc_size_weight
//       + popularity_contrib * $popularity_weight
//       + ($recency_numerator / (age_hours / 24.0 + $recency_smoothing)) * $recency_weight
//       + (pagerank * $pagerank_weight при включённом PageRank)
//       + (CASE языка поста vs User — как в feed_queries)
...
> В Neo4j нельзя ссылаться на только что введённый алиас `popularity_contrib` в том же `WITH`, что и выражение для `score`, поэтому в реализации используются **два** подряд `WITH` (как в примере выше).

> В режиме `global` вместо `log10(local_raw + …)` подставляется `log10(global_raw + $global_popularity_smoothing)`; в `blended` — смесь двух логарифмов с `$blend_local` / `$blend_global`, как в таблице выше.
...
WITH u, p,
     ...
     toFloat($reblogs_weight) * toFloat(COUNT { (p)<-[:REBLOGGED]-() }) +
     toFloat($replies_weight) * toFloat(COUNT { (p)<-[:REPLIED]-() }) +
     toFloat($bookmark_weight) * toFloat(COUNT { (p)<-[:BOOKMARKED]-() }) AS local_raw,
     toFloat($gf) * toFloat(COALESCE(p.totalFavourites, 0)) +
     toFloat($gr) * toFloat(COALESCE(p.totalReblogs, 0)) +
     toFloat($grp) * toFloat(COALESCE(p.totalReplies, 0)) AS global_raw,
...
WITH u, p, local_raw, global_raw, age_hours, pagerank,
     log10(local_raw + $popularity_smoothing) AS popularity_contrib
...
- Член `pagerank * $pagerank_weight` совпадает с персонализированной веткой при включённом PageRank.
...
| Параметр | CLI флаг | Env переменная | По умолчанию | Пример значения | Назначение |
|---------|----------|----------------|--------------|-----------------|-----------|
| feed_pc_share_weight | `--feed-pc-share-weight` | `HINTGRID_FEED_PC_SHARE_WEIGHT` | `0.45` | `0.45` | Вес доли интереса `share_i = i.score / sum_i` по всем `(uc)-[:INTERESTED_IN]->(pc)` (смягчение монополии одного PC при нескольких рёбрах) |
| feed_pc_size_weight | `--feed-pc-size-weight` | `HINTGRID_FEED_PC_SIZE_WEIGHT` | `0.05` | `0.05` | Вес нормализованного размера `norm_pc_size = pc.size / max(pc.size)` среди интересующих PC |
