...
- `get-post-info <ссылка>`: информация о посте по URL Mastodon, по публичному id из пути `/statuses/...` или по internal `statuses.id` (если такая строка есть в БД). См. SQL выше про различие id в URL и `statuses.id`.
